...
def init_multiuser_database():
    ...
    # Users table with admin controls
    c.execute('''
...
def get_user_from_token(authorization: Optional[str] = Header(None)) -> dict:
    ...
    c.execute('''
        SELECT u.id, u.email, u.name, u.is_active, u.is_admin, u.monthly_limit, s.expires_at
        FROM sessions s
...
def check_user_limit(user_id: int, monthly_limit: int):
    ...
    if count >= monthly_limit:
        raise HTTPException(
            status_code=429,
            detail=f"Monthly limit of {monthly_limit} operations reached. Contact administrator."
        )
...
def log_usage(user_id: int, action_type: str, cost_estimate: float = 0.03):
    ...
    c.execute('''
        INSERT INTO usage_log (user_id, action_type, timestamp, cost_estimate)
        VALUES (?, ?, ?, ?)
    ''', (user_id, action_type, datetime.now().isoformat(), cost_estimate))
...
def get_user_profile(user_id: int) -> dict:
    ...
    c.execute('''
        SELECT focus_areas, target_audience, content_goals, tone, monitoring_frequency
    ...
    return {
        "focus_areas": json.loads(result[0]),
        "target_audience": result[1],
        "content_goals": json.loads(result[2]),
        "tone": result[3],
        "monitoring_frequency": result[4]
    }
...
async def startup_event():
    ...
    try:
        ...
        scheduler.add_job(
            run_all_user_monitoring,
            CronTrigger(hour=8, minute=0, timezone='Europe/London'),
            id='daily_monitoring',
            name='Daily monitoring for all users',
...
async def signup(user: UserCreate):
    ...
    try:
        ...
        # Create user
        c.execute('''
            INSERT INTO users (email, password_hash, name, created_at, is_admin)
            VALUES (?, ?, ?, ?, ?)
        ''', (
            user.email,
        ...
        default_profile = {
            ...
            "target_audience": "Founders and leaders of scaling businesses",
            "content_goals": [
                "Provide actionable insights",
                "Share data-driven analysis",
        ...
        c.execute('''
            INSERT INTO user_profiles (user_id, focus_areas, target_audience, 
                                     content_goals, tone, monitoring_frequency)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            user_id,
            json.dumps(default_profile["focus_areas"]),
            default_profile["target_audience"],
            json.dumps(default_profile["content_goals"]),
            default_profile["tone"],
            default_profile["monitoring_frequency"]
        ))
...
async def login(credentials: UserLogin):
    ...
    c.execute('''
        SELECT id, name, password_hash, is_active, is_admin FROM users WHERE email = ?
    ''', (credentials.email,))
...
async def get_digest(
    ...
    """Get user's personalized digest"""
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    ...
    c.execute('''
        SELECT * FROM topics 
        WHERE user_id = ? AND created_at > ? AND status = 'new'
...
async def manual_monitoring(user: dict = Depends(get_user_from_token)):
    """Manually trigger monitoring for this user"""
    ...
    try:
        ...
        for suggestion in suggestions:
            ...
            ''', (
                ...
                json.dumps(suggestion.sources),
                json.dumps(suggestion.key_points),
...
async def brainstorm(
    ...
    try:
        ...
        conn = sqlite3.connect(db_path)
        c = conn.cursor()
        c.execute('SELECT * FROM topics WHERE id = ? AND user_id = ?', 
                 (request.topic_id, user["id"]))
        row = c.fetchone()
        conn.close()
        ...
        # Fake the brainstorm_post method to work with our topic data
...
async def update_topic_status(
    ...
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    c.execute('UPDATE topics SET status = ? WHERE id = ? AND user_id = ?',
             (status, topic_id, user["id"]))
    conn.commit()
    conn.close()
...
async def get_all_users(admin: dict = Depends(require_admin)):
    ...
    c.execute('''
        SELECT u.id, u.email, u.name, u.created_at, u.last_login,
               u.is_active, u.is_admin, u.monthly_limit,
               up.monitoring_frequency
        FROM users u
    ...
    for row in c.fetchall():
        ...
        users.append(AdminUserInfo(
            ...
            is_admin=bool(row[6]),
            monthly_limit=row[7],
            current_month_usage=usage_count,
            monitoring_frequency=row[8] or 'weekly'
        ))
...
async def update_user_admin(
    ...
    log_activity(admin["id"], "admin_update_user", 
                f"Updated user {user_id}: {updates.dict(exclude_none=True)}")
...
async def get_activity_log(
    ...
    c.execute('''
        SELECT a.timestamp, u.name, u.email, a.action, a.details
        FROM activity_log a
...
async def get_usage_stats(admin: dict = Depends(require_admin)):
    ...
    c.execute('SELECT COUNT(*), SUM(cost_estimate) FROM usage_log WHERE timestamp >= ?',
             (current_month_start.isoformat(),))
    api_calls, total_cost = c.fetchone()
    
    # Usage by action type
    c.execute('''
        SELECT action_type, COUNT(*), SUM(cost_estimate)
        FROM usage_log
        WHERE timestamp >= ?
    ...
    return {
        ...
        "estimated_cost_this_month": round(total_cost or 0, 2),
        "usage_by_type": usage_by_type
    }

def run_all_user_monitoring():
    """Background job to run monitoring for all users"""
...
c.execute('''
    SELECT u.id, u.is_active, u.monthly_limit, up.focus_areas, up.target_audience, 
           up.content_goals, up.tone, up.monitoring_frequency
    FROM users u
...
for user_row in users:
    ...
    # Check if we should run monitoring for this user today
    should_run = False
    if frequency == 'daily':
        should_run = True
    ...
    try:
        profile = {
            ...
            "content_goals": json.loads(content_goals),
            "tone": tone
        }
        ...
        c.execute('''
            INSERT INTO usage_log (user_id, action_type, timestamp, cost_estimate)
            VALUES (?, ?, ?, ?)
        ''', (user_id, "scheduled_monitoring", datetime.now().isoformat(), 0.03))
...
async def root():
return {
"message": "LinkedIn Content Assistant API - Multi-User with Admin",
"version": "2.0",
"features": ["authentication", "personalized_profiles", "admin_panel", "cost_controls"]
}
@app.get("/health")
...
uvicorn.run(app, host="0.0.0.0", port=port)</parameter>